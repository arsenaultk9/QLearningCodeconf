...
def optimize_model():
    ...
    state_encodings = torch.zeros((BATCH_SIZE, n_states))
    for batch_index in range(BATCH_SIZE):
        state_encodings[batch_index, batch.state[batch_index]] = 1   

    next_state_encodings = torch.zeros((BATCH_SIZE, n_states))
    for batch_index in range(BATCH_SIZE):
        next_state_encodings[batch_index, batch.next_state[batch_index]] = 1
    ...
    state_action_values = policy_net(state_encodings).gather(1, action_batch)
    ...
    with torch.no_grad():
        next_state_values = target_net(next_state_encodings).max(1).values
...
for episode in range(num_episodes):
    ...
    if show_episode:
        print(f"Episode average loss: {sum(episode_losses)/len(episode_losses)}")
